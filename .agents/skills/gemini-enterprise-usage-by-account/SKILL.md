...
description: >-
  Autopilot, hands-on diagnostics, and self-healing for gemini-enterprise-usage-by-account: Model Armor의 살균(Sanitize) 검사 로그를 BigQuery 로그 싱크로 실시간 적재하여, **엔터프라이즈 Gemini 환경에서 사용자별 프롬프트 검사 이력과 추정 토큰 소모량을...
---

# 제미나이 엔터프라이즈 Model Armor 감사 및 토큰 소모량 추정 (`gemini-enterprise-usage-by-account`) (Autopilot & Hands-on Guide)

본 스킬은 고객사 엔지니어와 실무자가 `gemini-enterprise-usage-by-account` 미니 프로젝트를 **"예습(YOLO 자율 주행) -> 실습(단계별 핸즈온) -> 복습(심화 검증 및 리소스 정리)"** 3단계 학습 사이클로 완주할 수 있도록 지원하는 실행 가이드다.
...
1. 현재 활성화된 gcloud 프로젝트 및 기본 리전을 확인한다:
   ...
   ```
2. `samples/gemini-enterprise-usage-by-account/.env.example`을 참조하여 `samples/gemini-enterprise-usage-by-account/.env` 파일이 없을 경우 자동 생성하고, 감지된 프로젝트 ID와 리전으로 기본 세팅을 구성한다.
3. 실행에 필요한 최소 IAM 권한(`roles/bigquery.admin, roles/logging.configWriter`) 충족 여부를 안내한다.
...
- **IAM 권한 부족 (`403 Forbidden`)**: 최소 필요 역할(`roles/bigquery.admin, roles/logging.configWriter`) 부여 가이드 제공
