...
1. 현재 활성화된 gcloud 프로젝트 및 기본 리전을 확인한다:
   ...
   ```
2. `samples/gemini-enterprise-domain-in-use-resolver/.env.example`을 참조하여 `samples/gemini-enterprise-domain-in-use-resolver/.env` 파일이 없을 경우 자동 생성하고, 감지된 프로젝트 ID와 리전으로 기본 세팅을 구성한다.
...
```bash
cd samples/gemini-enterprise-domain-in-use-resolver
./run.sh --dry-run
```
