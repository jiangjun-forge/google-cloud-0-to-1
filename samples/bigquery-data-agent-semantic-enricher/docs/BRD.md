...
## 4. 상세 기능 요구사항 명세 (Functional Requirements)

| 요구사항 ID | 기능 영역 | 상세 요구사항 및 비즈니스 목적 |
| :--- | :--- | :--- |
| **REQ-SEM-01** | 테이블 설명 감사 | 대상 데이터셋의 모든 테이블에 대해 AI 에이전트 라우팅에 충분한 길이와 의미를 가진 Table Description이 존재하는지 검사하여야 한다. |
| **REQ-SEM-02** | 컬럼 설명 감사 | 전체 컬럼 대비 명확한 설명이 기재된 컬럼의 비율(커버리지 %)을 산출하여 모호한 컬럼 존재 위험을 수치화하여야 한다. |
| **REQ-SEM-03** | 프로파일링 검증 | Dataplex Data Profile 스캔 수행 여부를 확인하여, 유효값 범위 및 고유값 목록이 LLM 그라운딩에 제공 가능한 상태인지 평가하여야 한다. |
| **REQ-SEM-04** | 용어집 공식 감사 | 주요 비즈니스 계산 메트릭에 대해 Dataplex Knowledge Catalog 용어집 수식(`Formula`)이 바인딩되어 있는지 감사하여야 한다. |
| **REQ-SEM-05** | 준비도 점수화 | 테이블 설명(30%), 컬럼 설명(30%), 프로파일(20%), 용어집(20%) 가중치를 합산하여 100점 만점의 Data Agent 준비도 점수를 산출하여야 한다. |
| **REQ-SEM-06** | 지능형 보강 계획 | 모호하거나 누락된 컬럼에 대해 Gemini 모델을 통해 실데이터 통계가 반영된 표준 설명 텍스트를 자동 생성하여야 한다. |
| **REQ-SEM-07** | 용어집 템플릿화 | 핵심 계산 수식(예: `SAFE_DIVIDE`, `subtotal - discount + tax`)을 포함한 Knowledge Catalog 배포용 YAML 파일을 자동 생성하여야 한다. |
| **REQ-SEM-08** | 스키마 패치 반영 | `--apply` 플래그 지정 시, 기존 데이터를 손상시키지 않고 BigQuery 테이블/컬럼 메타데이터만을 안전하게 패치(`client.update_table`)하여야 한다. |
| **REQ-SEM-09** | 가상 모의 실행 | 실제 GCP 자격 증명이나 BigQuery 데이터셋이 없는 환경에서도 결정론적 샘플 데이터로 진단 및 보강 효과를 시연할 수 있어야 한다. |

---

## 5. 비기능 요구사항 (Non-Functional Requirements)

1. **비파괴성 (Zero Risk)**: 기본 진단 모드는 순수 읽기 전용으로 동작하며, 메타데이터 반영(`--apply`) 시에도 실제 데이터 레코드는 일절 변경하지 않는다.
2. **실행 성능**: 100개 미만의 테이블 스키마 진단을 10초 이내에 완료한다.
3. **확장성**: 단일 테이블부터 대규모 전사 데이터 웨어하우스 데이터셋까지 동일한 로직으로 진단 및 보강을 지원한다.
