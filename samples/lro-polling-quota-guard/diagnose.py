...
def render_rca_and_prescription(res: Dict[str, Any], project_id: str, location: str) -> None:
    ...
    print(f"1. 대시보드 메트릭 착시 (The Metric Mirage):")
    print(f"   - 운영 대시보드에서는 주 작업 요청 메트릭(`{svc['job_method']}`)만 확인하므로 정상 한도 내로 표시된다.")
    print(f"   - 그러나 구글 파이썬 클라이언트 SDK 공통 기반 라이브러리인 `google-api-core`는")
    print(f"     `operation.result()` 호출 시 내부적으로 `GetOperation` API를 수초 간격으로 무차별 폴링한다.")
    print(f"2. 숨겨진 쿼터 고갈 메커니즘 (Hidden Quota Exhaustion):")
...
def main() -> None:
    ...
    print_diagnostic_banner(args.project, res["service_meta"]["name"], args.location, args.dry_run)
    render_assessment_table(res)
    render_rca_and_prescription(res, args.project, args.location)
