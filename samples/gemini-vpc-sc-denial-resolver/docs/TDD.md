...
# 기술 상세 설계서 (TDD): 제미나이(Gemini) VPC Service Controls 차단 감사 추적 및 처방 (`gemini-vpc-sc-denial-resolver`)
...
| :--- | :--- | :--- |
| **NFR-01** | **최소 IAM 권한 준수** | 진단에 필요한 최소 권한(`roles/iam.securityViewer, roles/logging.viewer`)만을 요구하며, 불필요한 쓰기 권한을 강제하지 않는다. |
