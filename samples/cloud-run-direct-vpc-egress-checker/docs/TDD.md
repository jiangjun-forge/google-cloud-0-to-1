...
# 기술 상세 설계서 (TDD): Cloud Run Direct VPC Egress 구성 및 네트워크 연결성 진단기
...
| :--- | :--- | :--- | :--- |
| **FR-01** | **BR-01** | **계층형 환경 설정 및 리소스 자동 탐지** | CLI 인자(`--dry-run, --project, --region, --service`) > `.env` > `gcloud config` 활성 설정 > 대화형 번호 선택기 순으로 프로젝트 및 대상 리소스를 자동 확정한다. |
...
| :--- | :--- | :--- |
| **NFR-01** | **최소 IAM 권한 준수** | 진단에 필요한 최소 권한(`roles/compute.networkViewer, roles/run.viewer`)만을 요구하며, 불필요한 쓰기 권한을 강제하지 않는다. |
