...
# 비즈니스 요구 사항 명세서 (BRD): Vertex AI Search 데이터 저장소 색인 및 그라운딩 정합성 진단기
...
사내 클라우드 운영 환경에서 인프라 담당자와 보안 및 아키텍처 실무자가 수동 콘솔 점검에 의존할 경우 설정 누락, 장애 인지 지연, 불필요한 비용 누수가 발생할 수 있다. 본 미니 프로젝트(`vertex-search-datastore-grounding-validator`)는 현장 실무에서 즉시 실행 가능한 자동화 진단 및 조치 가이드를 제공하여 운영 안정성을 체계적으로 확보한다.
...
| :--- | :--- | :--- | :--- |
| **BR-01** | **현장 장애 및 리스크 사전 탐지** | Vertex AI Search 데이터 저장소 색인 및 그라운딩 정합성 진단기 관련 운영 리스크 및 설정 누락을 사전에 식별해야 하는 경우 | 필수 (P0) |
...
- **보안 및 비용 거버넌스 담당자 (`#FinOps`, `#SecOps`)**: 최소 권한 원칙(`roles/discoveryengine.viewer, roles/storage.objectViewer`) 준수 및 리소스 과금 누수 예방
