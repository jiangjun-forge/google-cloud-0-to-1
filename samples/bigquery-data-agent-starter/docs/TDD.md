...
### 1.1 설계 목표
본 도구는 45분 핸즈온 워크숍 참석자가 BigQuery 스몰셋 데이터셋(`cymbal_gold`) 구축부터 BigQuery Data Agent 단독(Standalone) 생성, 선택적 Gemini Enterprise App 연동, 그리고 `bigquery-data-agent-semantic-enricher`와의 Before/After 정확도 비교 시연까지 수행할 수 있도록 지원하는 **경량 워크숍 프로비저닝 및 연동 가이드 엔진**이다.
...
3. **스키마 호환성 및 독립성(Schema Compatibility with Zero Dependency)**: `bigquery-data-agent-semantic-enricher`와 코드 의존성은 일절 두지 않으면서도 동일한 `cymbal_gold` 4개 테이블 스키마를 생성하여, 스타터 실습 직후 메타데이터 보강 전후 비교 데모를 원활히 지원.
...
flowchart TD
    ...
    subgraph BigQuery_CymbalGold["BigQuery 호환 데이터 패브릭 (cymbal_gold)"]
        RawTables["4개 원천 테이블 (pos_transactions_gold 등 - 설명 누락 상태)"]
    ...
    subgraph Optional_Extensions["선택적 확장 및 비교 시연 (Optional)"]
        GEApp["Gemini Enterprise App (--register-ge-app)"]
        Enricher["bigquery-data-agent-semantic-enricher (--enrich --apply)"]
    end
    ...
    RawTables -.->|4. Before/After 비교 시연| Enricher
```
...
| **FR-03** | `export_data_agent_config` | 순매출 및 결품 커버 시간 공식이 명시된 System Instructions와 Verified Queries를 출력하고 Before/After 비교 시연 방법을 안내하여야 한다. |
...
| **NFR-03** | 코드 독립성 | 타 샘플(`bigquery-data-agent-semantic-enricher`)의 모듈이나 파일을 참조하지 않고 100% 자체 완결적으로 실행된다. |
