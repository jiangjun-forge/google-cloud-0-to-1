...
============================================================================================
  * Vertex AI Gemini API 첫 토큰 수신 (TTFT) :    720.5 ms (스트리밍 즉시 수신)
  * Vertex AI Gemini API 전체 응답 완료 시간  :   1840.2 ms
  * Gemini Enterprise App 완결 E2E 소요 시간  :   4785.4 ms (보안 가드레일 + 사내 RAG)
--------------------------------------------------------------------------------------------
  * 1) 첫 토큰 수신 시점 차이 : GE App 파이프라인에서 약 6.6배 추가 시간 소요
       (선행 조건: Model Armor 인스펙션 및 사내 인덱스 검색 완료 후 토큰 생성 착수)
  * 2) 전체 E2E 완료 시간 차이 : GE App 파이프라인에서 약 2.6배 소요 (+2945.2 ms)
============================================================================================
```

---

## 6. 아키텍처 관점의 파이프라인 지연 시간 차이 분석

1. **초기 응답 지연 시간(TTFT) 구조 차이**:
   - **순수 Vertex AI API**: 사용자의 요청 시 선행 보안 필터나 지식 검색 단계 없이 **0.5~0.8초 내에 첫 번째 토큰이 스트리밍 전송**되어 초기 응답 시점이 빠르다.
   - **GE App**: Model Armor의 악성 프롬프트 인스펙션(약 0.7초) 및 사내 데이터스토어 인덱스 검색과 IAM/ACL 권한 필터링(약 2초)이 모두 완료된 후에 비로소 LLM 토큰 생성이 시작되므로, 초기 토큰 노출 시점까지 대기 구간이 발생한다.
2. **사내 문서 RAG 요약 및 사후 보안 검증**:
   - 검색된 사내 문서를 컨텍스트에 주입하여 답변을 요약 생성하는 RAG 연산과, 생성된 응답에 대한 사후 민감정보(PII/SDP) 검사(약 0.6초)가 동기식으로 실행되어 전체 파이프라인 소요 시간이 누적된다.
3. **아키텍처 결론**:
   - 양 파이프라인 간의 지연 시간 차이는 모델 자체의 추론 성능 문제가 아니며, **사내 비공개 데이터 기반 그라운딩(Grounding)과 기업 보안 거버넌스(Model Armor)가 동기식으로 작동하는 구조적 차이**에서 비롯된다.
