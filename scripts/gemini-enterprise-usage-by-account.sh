...
# 용도: 제미나이 엔터프라이즈(Gemini Enterprise) 앱 로그를 빅쿼리(BigQuery)로 자동 적재하고, SQL 쿼리를 통해 사용자별 사용 횟수를 산출해 모니터링하기 위함이다.
...
# 제미나이 엔터프라이즈 앱 로그 수집을 위해 Vertex AI 데이터 액세스 감사 로그를 활성화한다.
...
# 실시간으로 들어오는 제미나이 엔터프라이즈 앱 로그만 필터링하는 로깅 쿼리 언어(Logging Query Language, LQL) 쿼리다.
# 로그 쿼리 콘솔 주소: https://console.cloud.google.com/logs/query
#
# 로깅 쿼리 원문: resource.type = "gemini_enterprise_resource"
