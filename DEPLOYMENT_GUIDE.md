...
```mermaid
flowchart LR
    A["1️⃣ 감사 로그 활성화"] --> B["2️⃣ Cloud Logging 필터 확인"]
    B --> C["3️⃣ Log Sink → BigQuery"]
    C --> D["4️⃣ BigQuery SQL 조회"]
```
...
> [!IMPORTANT]
> 아래 모든 명령에서 `your-project-id`를 **실제 Google Cloud 프로젝트 ID**로 반드시 변경하세요.
...
> [!WARNING]
> `set-iam-policy`는 기존 정책을 **덮어쓰기**합니다. 반드시 기존 정책을 먼저 다운로드한 후 `auditConfigs`만 추가/수정하세요.
...
> [!TIP]
> 로그가 보이지 않는다면 실제 Gemini 사용(API 호출 또는 Console에서 Gemini 기능 사용)을 한 뒤 **1~2분 후** 다시 확인하세요.
...
> [!NOTE]
> 두 스크립트가 동일한 `LOG_SINK_NAME`과 `BIGQUERY_DATASET_ID`를 사용합니다. 두 종류의 로그를 **모두** 수집하려면 싱크 이름을 다르게 설정하거나, 하나의 싱크에서 두 필터를 OR 조건으로 합치세요.
...
> [!TIP]
> 통합 싱크(옵션 B)를 선택한 경우 싱크 이름을 `gcp-logs-sink`로 변경하여 한 번만 실행하면 됩니다.
...
flowchart TB
    ...
    subgraph GCP["Google Cloud Project"]
        subgraph GeminiServices["Gemini Services"]
            API["Vertex AI / Gemini API"]
            ENT["Gemini Enterprise App<br/>(Cloud Console 내장)"]
        end

        subgraph Logging["Cloud Logging"]
            AL["Data Access<br/>Audit Logs"]
            SINK["Log Router Sink"]
        end

        subgraph Analytics["BigQuery"]
            DS["Dataset: gcp_logs"]
            TBL["cloudaudit_googleapis_com_<br/>data_access_*"]
        end
    end
    ...
    SINK -->|"자동 적재"| DS
    DS --> TBL
    TBL -->|"SQL 쿼리"| REPORT["📈 사용자별<br/>사용량 리포트"]
```
